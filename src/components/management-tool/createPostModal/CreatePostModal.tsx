...
export function CreatePostModal({
  ...
  const [scheduleDate, setScheduleDate] = React.useState<Date | null>(selectedDate || new Date());
        ...
        <div className="space-y-4 pt-7">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between border-t pt-4 gap-2">
            <div className="flex flex-wrap items-center pt-4 gap-2">
              <span className="text-sm whitespace-nowrap">{t('scheduleFor')}:</span>
              <div className="flex w-full gap-2 sm:w-auto">
                <Input
                  type="date"
                  value={scheduleDate ? format(scheduleDate, 'yyyy-MM-dd') : ''}
                  onChange={(e) => setScheduleDate(new Date(e.target.value + 'T00:00:00'))}
                  className="w-full"
                />
                <Input
                  type="time"
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-end gap-2 pt-4">
            ...
            {
              editingPost && (
                <Button onClick={handleDeletePost} className="border border-red-500 text-red-500 hover:bg-red-500 hover:text-white active:bg-red-600">
                  {t('delete')}
                </Button>
              )
            }
