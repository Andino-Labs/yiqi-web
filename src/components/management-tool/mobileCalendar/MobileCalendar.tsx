...
import { format, addDays, startOfToday, subDays, isSameDay, isBefore } from "date-fns"
...
export function MobileCalendarView({ onOpenModal, posts, onEditPost }: MobileCalendarViewProps) {
    ...
    return (
        <div className="space-y-6">
            ...
            {days.map((day) => {
                ...
                const sortedPosts = dayPosts.sort((a, b) => new Date(a.scheduledDate).getTime() - new Date(b.scheduledDate).getTime());
                        ...
                        <div className="space-y-2">
                            {sortedPosts.length > 0 ? (
                                sortedPosts.map(post => (
                                    <Card
                                        key={post.id}
                                        className={`flex items-center justify-between p-4 cursor-pointer ${post.status === "PUBLISHED"
                                                ? "bg-green-500 hover:bg-green-600"
                                                : "bg-blue-500 hover:bg-blue-600"
                                            }`}
                                        onClick={() => { if(isSelectable) { onEditPost(post)}}}
                                    >
                                        <div className="text-left flex-grow">
                                            <time className="text-lg">{format(new Date(post.scheduledDate), 'HH:mm')}</time>
                                            <p className="text-sm truncate">{post.content.substring(0, 20)}...</p>
                                        </div>
                                    </Card>
                                ))
                            ) : isPastDay ? (
                                ...
                                <Card
                                    ...
                                    onClick={() => { if(isSelectable) { onOpenModal(day)}}}
                                >
